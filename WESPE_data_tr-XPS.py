...
waterfall_delay_cut = 'off'  # Set 'on' to get >=0 difference between two curves
...
for i in run_numbers:
    if i == run_numbers[0]:  # excecuted for the first scan from the list
        ...
        try:
            ...
            run_numbers[run_numbers.index(i)] = f'{i}_static ({static} ps)'
            t.insert(0, static)
            dt.insert(0, 0.5)
            static += 1
        ...
        try:  # check if we have a static scan
            ...
            run_numbers[run_numbers.index(i)] = f'{i}_static ({static} ps)'
            t.insert(0, static)  # we force the hor. cut panel to show the scan
            dt.insert(0, 0.5)
            static += 1
